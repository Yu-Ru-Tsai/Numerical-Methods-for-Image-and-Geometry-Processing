function DenoisedImg = SplitBregman12norm(img_noisy, linear_solver_method, ...
...
A  = mu*(speye(N) + nablaX.'*nablaX + nablaY.'*nablaY) + alpha*speye(N);
...
while (OUTER_ERROR > OUTER_TOL && iterations < OUTER_MAX_ITER) || iterations <= OUTER_MIN_ITER
    ...
    rhs = mu*(f - d + b1 + nablaX.'*(x-b2) + nablaY.'*(y-b3)) + alpha*f;
...
DenoisedImg = uint8(reshape(u,m,n)*255);