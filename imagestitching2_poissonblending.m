...
Ifile = imageDatastore(fullfile('footage', 'classroom302-4k'));
...
[alllines(1,:)]=linefunc(Xd,Yd,Xu,Yu);%midline
% [alllines(2,:)]=linefunc(x2(1),y2(1),floor(Xu),ceil(Yu));%leftupline
% [alllines(3,:)]=linefunc(floor(Xu),ceil(Yu),x1(3),y1(3));%rightupline
% [alllines(4,:)]=linefunc(x1(3),y1(3),x1(4),y1(4));%rightline
% [alllines(5,:)]=linefunc(x1(4),y1(4),floor(Xd),floor(Yd));%rightdownline
% [alllines(6,:)]=linefunc(floor(Xd),floor(Yd),x2(2),y2(2));%leftdownline
% [alllines(7,:)]=linefunc(x2(2),y2(2),x2(1),y2(1));%leftline
...
VI = find(maskmix == 1); % change indexmask to VI
VB = setdiff(1:masksize(1)*masksize(2), VI).';

maskmixx = maskmix;

[VBy, VBx] = ind2sub(masksize, VB);
maskmixx(VB(alllines(1,1)*VBx+alllines(1,2)*VBy+alllines(1,3)<0))=0;%right VB
maskmixx(VB(alllines(1,1)*VBx+alllines(1,2)*VBy+alllines(1,3)>0))=1;%left VB

maskmixx = maskmixx(:);
%%
L = LatticeLaplacian(masksize(1), masksize(2));
rhs = -L(VI,VB)*maskmixx(VB,:);
% maskmixx(VI,:) = L(VI,VI)\rhs;
% w = 1.2;
% [M1, M2] = SSOR_Precond(L(VI,VI), w);
pfun = cmg_sdd(-L(VI,VI));
% kkk = size(rhs);
% maskmixx(VI,:) = LinearSysSolver('ssor', L(VI,VI), rhs, zeros(kkk(1), 1), 300, 1e-16, M1, M2);
[maskmixx(VI,:), flag, error, iter] = pcg(-L(VI,VI), -rhs, 1e-3, 3, pfun);
% maskmixx(VI, :) = ConjugateGradientMethod(L(VI, VI), rhs, randn(kkk(1), 1), 5000, 1e-8); 
% flag
% error
% iter

maskright = reshape(maskmixx(:),masksize(1), masksize(2));
maskright(VB) = 0;
figure;imshow(maskright);


maskleft = maskright;
maskleft(VI) = 1 - maskright(VI);
...
panorama = panorama + warpedImage1 .* maskright;%figure;imshow(panorama);
panorama = panorama + warpedImage2 .* maskleft;figure;imshow(panorama);
