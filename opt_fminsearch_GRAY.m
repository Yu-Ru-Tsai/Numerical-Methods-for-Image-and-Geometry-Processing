...
for model_num = 1 : length(model_name)
    ...
    if strcmp(model_name(model_num), "1-Norm") || strcmp(model_name(model_num), "Isotropic") ...
        ...
        option = optimset('PlotFcns',@optimplotfval);
        ...
        optimal_parameters(model_num, 1:2) = fminsearch(@(x)(ssim(WhichModel(model_name(model_num), img_noisy_temp, linear_solver_method, x(1), x(2), 0, ...
            OUTER_MIN_ITER, OUTER_MAX_ITER, OUTER_TOL, OUTER_ERROR, INNER_MAX_ITER, INNER_TOL), img_origin)*-1), x0, option);  
    ...
    elseif strcmp(model_name(model_num), "2-Norm") || strcmp(model_name(model_num), "1->2-Norm")
        ...
        optimal_parameters(model_num, 2) = fminsearch(@(x)(ssim(WhichModel(model_name(model_num), img_noisy_temp, linear_solver_method, 0, x, 0, ...
        0, 0, 0, 0, INNER_MAX_ITER, INNER_TOL), img_origin)*-1), x0, option);  
    ...
    elseif strcmp(model_name(model_num), "1&2-Norm")
        ...
        optimal_parameters(model_num, 1:3) = fminsearch(@(x)(ssim(WhichModel(model_name(model_num), img_noisy, linear_solver_method, x(1), x(2), x(3), ...
            OUTER_MIN_ITER, OUTER_MAX_ITER, OUTER_TOL, OUTER_ERROR, INNER_MAX_ITER, INNER_TOL), img_origin)*-1), x0, option);  
...
result_name = append('result/opt_fminsearch_', name, '_', upper(string(noise_type)), '_GRAY_ssim.png');
