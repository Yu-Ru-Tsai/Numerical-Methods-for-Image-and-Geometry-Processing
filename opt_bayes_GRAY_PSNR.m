...
for model_num = 1 : length(model_name)
    ...
    if strcmp(model_name(model_num), "1-Norm") || strcmp(model_name(model_num), "Isotropic") ...
        ...
        while(optimal_parameters(model_num, 2) == 1 || round(optimal_parameters(model_num, 2), 1) == mu_upperbound)
            ...
            vars0 = table(1, optimal_parameters(model_num, 2)); % initial values
        
            if strcmp(model_name(model_num), "2->1-Norm") % changing the noisy image with 2-norm denoised image
    
                [~, max_index] = max([PSNR(reshape(img_denoised(2,:,:), [img_size_h, img_size_w]), img_origin), ...
                    PSNR(reshape(img_denoised(3,:,:), [img_size_h, img_size_w]), img_origin), ...
                    PSNR(reshape(img_denoised(4,:,:), [img_size_h, img_size_w]), img_origin)]); % selecting the highest PSNR denoised image
            ...
            results = bayesopt(@(vars)(PSNR(WhichModel(model_name(model_num), img_noisy_temp, linear_solver_method, vars.lambda, vars.mu, 0, ...
    ...
    elseif strcmp(model_name(model_num), "2-Norm") || strcmp(model_name(model_num), "1->2-Norm")
        ...
        while(optimal_parameters(model_num, 2) == 1 || round(optimal_parameters(model_num, 2), 1) == mu_upperbound)
            ...
            results = bayesopt(@(vars)(PSNR(WhichModel(model_name(model_num), img_noisy_temp, linear_solver_method, 0, vars.mu, 0, ...
    ...
    elseif strcmp(model_name(model_num), "1&2-Norm")
        ...
        while(optimal_parameters(model_num, 2) == 1 || round(optimal_parameters(model_num, 2), 1) == mu_upperbound)
            ...
            vars0 = table(1, optimal_parameters(model_num, 2), 1); % initial values
        
            results = bayesopt(@(vars)(PSNR(WhichModel(model_name(model_num), img_noisy, linear_solver_method, vars.lambda, vars.mu, vars.alpha, ...
                OUTER_MIN_ITER, OUTER_MAX_ITER, OUTER_TOL, OUTER_ERROR, INNER_MAX_ITER, INNER_TOL), img_origin)*-1), vars, 'PlotFcn', [], ...
                'MaxObjectiveEvaluations', EVALUTION_HYPERPARAMETER, 'UseParallel', true, 'InitialX', vars0);        
...
result_name = append('result/opt_bayes_', name, '_', upper(string(noise_type)), '_GRAY_PSNR.png');
